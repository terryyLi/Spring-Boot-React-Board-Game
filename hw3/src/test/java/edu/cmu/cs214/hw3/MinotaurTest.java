...
public class MinotaurTest {
    @Test
    public void moveIntoOpponentDiagnal() {
        Board board1 = new Board(5, 5, 4);
        Player player1 = new Minotaur(new Worker(new Position(0, 0), 1), new Worker(new Position(1, 0), 2), board1, 1);
        Player player2 = new Minotaur(new Worker(new Position(0, 1), 1), new Worker(new Position(1, 1), 2), board1, 2);
        ...
        assertEquals(player2.getWorker2().getPosition().getX(), 2);
        assertEquals(player2.getWorker2().getPosition().getY(), 2);
    }

    @Test
    public void moveIntoOpponentHorizontal() {
        Board board1 = new Board(5, 5, 4);
        Player player1 = new Minotaur(new Worker(new Position(0, 0), 1), new Worker(new Position(1, 0), 2), board1, 1);
        Player player2 = new Minotaur(new Worker(new Position(0, 1), 1), new Worker(new Position(1, 1), 2), board1, 2);
        boolean move = player1.move(player1.getWorker1(), new Position(0, 1));
        assertTrue(move);
        assertEquals(player1.getWorker1().getPosition().getX(), 0);
        assertEquals(player1.getWorker1().getPosition().getY(), 1);
        assertEquals(player2.getWorker1().getPosition().getX(), 0);
        assertEquals(player2.getWorker1().getPosition().getY(), 2);
    }
    ...
    public void simplyBuild() {
        ...
        boolean build2 = player1.build(player1.getWorker1(), new Position(1, 1));
        assertTrue(build2);
        assertEquals(player1.getBoard().getTowerLevel(new Position(1, 1)), 2);
    }
